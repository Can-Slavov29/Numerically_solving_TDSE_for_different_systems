...
Infinite square well was simple as no potential within the well and 0 outside means you only need to consider the grid within the well itself. The other two required more thought both about the paramaters chosen as they would affect wether or not the wavefunction decayed properly and wether the behaviour was accurately captured, but also having more complex potentials meant figuring out how to create those matrices.

Some general things that needed to be considered throughout were: size of N as larger N allowed for accuracy but also longer runtime, wether or not to use natural units(c=h_bar=1) or physical ones. In the end I opted for real physical ones as it felt more intuitive. The latter required research for "typical" depths/widths of wells etc.
